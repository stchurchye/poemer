...
import { estimateTokens, getContextWindowTokens, getCompactCompletionMaxTokens, getCompactSummaryMaxTokens, trimTextToTokenBudget, assembleChatContext, assembleWritingExecuteContext, assembleWritingIntentContext, COMPACT_SUMMARY_MAX_TOKENS, } from './contextBudget.js';
...
test('assembleWritingExecuteContext: pinned 内容自身超过窗口时明确拒绝', () => {
    assert.throws(() => assembleWritingExecuteContext({
        systemPrompt: 'system',
...
test('assembleChatContext: pendingUser 自身超过窗口时明确拒绝', () => {
    assert.throws(() => assembleChatContext({
        systemPrompt: 'system',
        history: [],
...
test('assembleWritingIntentContext: 超长当前章节会裁到窗口内', () => {
    ...
    assert.ok(result.usage.usedTokens <= result.usage.limitTokens);
    assert.ok(result.messages.reduce((sum, message) => sum + estimateTokens(message.content), 0) + 100 <=
        1000);
});
test('assembleWritingIntentContext: 用户指令自身超过窗口时明确拒绝', () => {
    assert.throws(() => assembleWritingIntentContext({
        systemPrompt: 'system',
        history: [],
        chapterBlock: '正文',
        documentBlock: '',
