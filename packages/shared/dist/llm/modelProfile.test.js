...
import { getModelProfile, DEFAULT_MODEL_PROFILE, listKnownModelIds, } from './modelProfile.js';
...
test('flash-lite window is far larger than gpt window (保原文: 多留逐字)', () => {
    assert.ok(getModelProfile('google/gemini-3.1-flash-lite').contextWindowTokens >
        getModelProfile('openai/gpt-5.4').contextWindowTokens);
    assert.ok(listKnownModelIds().length >= 2);
});
//# sourceMappingURL=modelProfile.test.js.map