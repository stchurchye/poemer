...
import { blocksFromAssembleChatResult, blocksFromWritingIntent, } from './contextPreview.js';
import { assembleChatContext, assembleWritingIntentContext, } from './contextBudget.js';
...
test('blocksFromAssembleChatResult: 有历史被预算裁掉时，历史块 messageId 对齐到 fitted 尾部而非从头', () => {
    ...
    const historyBlocks = preview.blocks.filter((b) => (b.kind === 'history_user' || b.kind === 'history_assistant') && !b.omittedByBudget);
    ...
    assert.equal(historyBlocks[historyBlocks.length - 1].messageId, ids[ids.length - 1], '最后一个 fitted 块 id 应为最新消息');
});
...
test('preview block ids stay stable when summary visibility changes', () => {
    ...
    const withoutSummary = blocksFromAssembleChatResult(assembleChatContext({
        systemPrompt: 'system',
        history,
        pendingUser: '继续',
        limitTokens: 10_000,
    }), { historyMessageIds: ids });
    const withSummary = blocksFromAssembleChatResult(assembleChatContext({
        systemPrompt: 'system',
        summary: '摘要',
        history,
        pendingUser: '继续',
    ...
    const chatIds = (preview) => preview.blocks
    ...
    const writingWithoutSummary = blocksFromWritingIntent(assembleWritingIntentContext({
        systemPrompt: 'system',
        history,
        chapterBlock: '章节',
        documentBlock: '全文',
    ...
    const writingWithSummary = blocksFromWritingIntent(assembleWritingIntentContext({
        systemPrompt: 'system',
        summary: '摘要',
        history,
        chapterBlock: '章节',
    ...
    const writingIds = (preview) => preview.blocks
...
//# sourceMappingURL=contextPreview.test.js.map