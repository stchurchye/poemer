...
export function computeDiff(oldText, newText) {
    ...
    while (i < o.length || j < n.length) {
        if (i < o.length && j < n.length && o[i] === n[j]) {
            ...
            if (buf)
                segments.push({ type: 'equal', text: buf });
            continue;
        }
        if (j < n.length && (i >= o.length || o[i] !== n[j])) {
            ...
            while (j < n.length && (i >= o.length || o[i] !== n[j])) {
                buf += n[j];
                j++;
                if (i < o.length && o[i] === n[j])
                    break;
            }
            if (buf)
                segments.push({ type: 'insert', text: buf });
            continue;
        }
        if (i < o.length) {
            let buf = '';
            ...
            if (buf)
                segments.push({ type: 'delete', text: buf });
        }
    }
...
export function diffSummary(segments) {
    let inserts = 0;
    let deletes = 0;
    for (const s of segments) {
        if (s.type === 'insert')
            inserts += s.text.length;
        if (s.type === 'delete')
            deletes += s.text.length;
    }
    const parts = [];
    if (inserts > 0)
        parts.push(`新加了${inserts}个字`);
    if (deletes > 0)
        parts.push(`删去了${deletes}个字`);
    if (parts.length === 0)
        return '内容没有变化';
