...
import {
  loadPersistedStoreFiles,
  savePersistedStoreFiles,
  type PersistedFileOperations,
  type PersistedStoreFilePaths,
} from './persistedFileStore.js';

function memoryOperations(initial: Array<[string, string]>) {
  const files = new Map<string, string>(initial);
  const copies: Array<{ from: string; to: string }> = [];
  const operations: PersistedFileOperations = {
    async exists(path) {
...
test('backup recovery never overwrites the valid backup with a broken main file', async () => {
  const paths: PersistedStoreFilePaths = {
    main: 'main.json',
    backup: 'backup.json',
  ...
  assert.ok(
    !copies.some((copy) => copy.from === paths.main && copy.to === paths.backup),
    '损坏主文件绝不能覆盖最后一份有效备份',
  );
});
...
test('failed recovery write leaves the valid backup untouched and reports failure', async () => {
  ...
  fixture.operations.write = async (path, value) => {
    if (path === paths.main) throw new Error('disk unavailable');
    await write(path, value);
  };

  await assert.rejects(
    loadPersistedStoreFiles(fixture.operations, paths),
    /disk unavailable/,
  );
...
test('main read failure falls back to a valid backup', async () => {
  ...
  fixture.operations.read = async (path) => {
    if (path === paths.main) throw new Error('EIO');
    return read(path);
  };
...
test('temporary read failures for both main and backup never create an empty store', async () => {
  ...
  fixture.operations.read = async (path) => {
    if (path === paths.main || path === paths.backup) throw new Error(`EIO:${path}`);
...
test('normal save never rotates an invalid main over a valid backup', async () => {
  ...
  await assert.rejects(
    savePersistedStoreFiles(fixture.operations, paths, createEmptyPersistedStore()),
    /disk unavailable/,
  );
  assert.equal(fixture.files.get(paths.backup), validBackup);
});
