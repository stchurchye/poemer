...
export const STORY_BIBLE_CATEGORY_LABELS: Record<StoryBibleCategory, string> = {
  character: '人物',
  term: '称呼/词条',
  timeline: '时间线',
  style: '风格',
...
function entryKey(category: StoryBibleCategory, label: string): string {
  return `${category}:${normalizeLabel(label)}`;
}

/** 由 category+label 生成稳定 id（同一条目多次抽取 id 不变，天然幂等去重） */
function stableId(category: StoryBibleCategory, label: string): string {
  return `sb-${entryKey(category, label)}`;
}

/** 把设定卡格式化成写作上下文里的常驻块；空则返回空串 */
export function formatStoryBibleForLlm(bible?: StoryBible | null): string {
  const entries = bible?.entries?.filter((e) => e?.label?.trim() && e?.note?.trim());
  if (!entries || entries.length === 0) return '';

  const byCategory = new Map<StoryBibleCategory, StoryBibleEntry[]>();
  ...
  const lines: string[] = [STORY_BIBLE_BLOCK_PREFIX];
...
export function mergeStoryBible(
  existing: StoryBible | null | undefined,
  incoming: StoryBible,
): StoryBible {
  ...
  for (const e of incoming.entries) {
    if (!e?.label?.trim() || !e?.note?.trim()) continue;
    map.set(entryKey(e.category, e.label), {
      id: stableId(e.category, e.label),
      category: e.category,
      label: e.label.trim(),
...
export function parseStoryBibleEntries(raw: string): StoryBibleEntry[] {
  ...
  for (const item of arr.slice(0, MAX_STORY_BIBLE_ENTRIES * 2)) {
    ...
    const cat = category as StoryBibleCategory;
    out.push({
...
function extractJsonArray(raw: string): unknown[] | null {
  let best: { arr: unknown[]; objCount: number } | null = null;
...
function matchBalancedArrayEnd(raw: string, start: number): number {
  let depth = 0;
  let inStr = false;
  let esc = false;
