...
test('blocksFromAssembleChatResult: 被排除的普通消息仍保留为未选候选块', () => {
  const availableHistory = [
    { id: 'm-private', role: 'user' as const, content: '不发送的私密内容' },
    { id: 'm-public', role: 'assistant' as const, content: '可以发送的回答' },
  ];
  const assembled = assembleChatContext({
    systemPrompt: 'system',
...
test('blocksFromWritingIntent: 被排除的写作消息仍保留为未选候选块', () => {
  const availableHistory = [
    { id: 'w-private', role: 'user' as const, content: '不发送的写作私密内容' },
    { id: 'w-public', role: 'assistant' as const, content: '可以发送的写作回答' },
  ];
