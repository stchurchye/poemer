 ...
 */
export function stripChatReplyCitations(text: string): string {
  let s = text.trim();
  if (!s) return s;
...
function stripInlineWebCitations(s: string): string {
  let out = s;
...
function cleanupOrphanPunctuation(s: string): string {
  return s
    .replace(/\(\s*\)/g, '')
    .replace(/([，。！？；、：])\s*\)+/g, '$1')
    .replace(/\(\s*([，。！？；、：])/g, '$1')
