import { isAssistantGuideKey, type AssistantGuideKey } from './assistantGuideRegistry.js';
...
function parseMode(value: unknown): 'chat' | 'revise' | 'guide' {
  if (value === 'guide') return 'guide';
  return value === 'revise' ? 'revise' : 'chat';
}

function parseReferenceScope(value: unknown): 'chapter' | 'document' {
...
function looksLikeIntentJson(j: Record<string, unknown>): boolean {
  return (
    j.mode !== undefined ||
    j.ready !== undefined ||
    j.action !== undefined ||
...
function mapIntent(j: Record<string, unknown>, prose: string): ParsedWritingIntent {
...
export function parseWritingIntentResponse(raw: string): ParsedWritingIntent {
...
export function stripWritingIntentDisplayText(content: string): string {
