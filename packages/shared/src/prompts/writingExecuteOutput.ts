...
function tryParseBasisJson(text: string): { evaluation?: string; rationale?: string } | null {
  const trimmed = text.trim();
  if (!trimmed.startsWith('{')) return null;
  try {
    return JSON.parse(trimmed) as { evaluation?: string; rationale?: string };
  } catch {
...
function basisFromJson(j: { evaluation?: string; rationale?: string }): WritingExecuteBasis | null {
  const evaluation = j.evaluation?.trim() ?? '';
  const rationale = j.rationale?.trim() ?? '';
  if (!evaluation && !rationale) return null;
  return { evaluation, rationale };
}

export function hasWritingExecuteBasis(basis: WritingExecuteBasis | null | undefined): boolean {
  return Boolean(basis?.evaluation?.trim() || basis?.rationale?.trim());
}

/** 保证写入 revision / 消息时一定有可展示的改稿依据 */
export function ensureWritingExecuteBasis(
  basis: WritingExecuteBasis | null | undefined,
  action: string,
): WritingExecuteBasis {
...
export function parseWritingExecuteResponse(raw: string): {
  text: string;
  basis: WritingExecuteBasis | null;
} {
  const trimmed = raw.trim();
  if (!trimmed) return { text: '', basis: null };
