...
export function filterWritingMessagesForContext(
  ...
  for (let i = 0; i < messages.length; i++) {
    const m = messages[i];
    if (m.kind !== 'intent_confirm' || m.confirmStatus !== 'rejected') continue;

    excludedIds.add(m.id);
    const prev = messages[i - 1];
    if (prev?.role === 'user') excludedIds.add(prev.id);
...
export const WRITING_CHAPTER_ONLY_SCOPE_NOTE =
  '（用户要求仅根据本章理解，已省略其它章节正文）';

export const WRITING_FULL_ARTICLE_BLOCK_PREFIX =
  '整篇文章按章列出（供理解；评价与改稿仍只针对「当前待改章节」）：';
 ...
 */
export function formatArticleChaptersForLlm(
  doc: Document,
...
export function buildWritingChapterBlock(chapterTitle: string, chapterContent: string): string {
...
export function buildWritingDocumentBlock(documentExcerpt: string): string {
...
}): { chapterBlock: string; documentBlock: string } {
  const chapterBlock = buildWritingChapterBlock(
    input.chapterTitle,
