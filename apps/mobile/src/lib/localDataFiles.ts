...
import {
  ...
  type PersistedStore,
  type PersistedStoreFileLoadResult,
...
export type LocalDataLoadResult = PersistedStoreFileLoadResult;
...
const STORE_PATHS: PersistedStoreFilePaths = {
  main: STORE_FILE,
  backup: BACKUP_FILE,
  temporary: TEMP_FILE,
  broken: BROKEN_FILE,
...
const FILE_OPERATIONS: PersistedFileOperations = {
  async exists(path) {
    return (await FileSystem.getInfoAsync(path)).exists;
  },
  read: (path) => FileSystem.readAsStringAsync(path),
...
export async function saveLocalPersistedStore(store: PersistedStore): Promise<void> {
  await savePersistedStoreFiles(FILE_OPERATIONS, STORE_PATHS, store);
}

export async function loadLocalPersistedStore(): Promise<LocalDataLoadResult> {
  return loadPersistedStoreFiles(FILE_OPERATIONS, STORE_PATHS);
}
